...
class SourcePath(
    ...
    private inner class SourceFile(
            ...
            val isTemporary: Boolean =
                    false, // A temporary source file will not be returned by .all()
    ...
    private fun refreshWorkspaceIndexes(oldFiles: List<SourceFile>, newFiles: List<SourceFile>) =
            indexAsync.execute {
                if (indexEnabled) {
                    ...
                    // Index the new declarations in the Kotlin source files that were just
                    // compiled, removing the old ones
    ...
    private fun refreshDependencyIndexes(module: ModuleDescriptor) =
            indexAsync.execute {
                if (indexEnabled) {
                    ...
                    // Fix: Disable external dependency indexing to avoid infinite progress bar
                    // ("Kotlin: Indexing").
                    // `module.getSubPackagesOf` takes exponentially longer on Android/Gradle
                    // Classpaths and often hangs.
                    // Workspace indexing (`refreshWorkspaceIndexes`) remains fully functional.
