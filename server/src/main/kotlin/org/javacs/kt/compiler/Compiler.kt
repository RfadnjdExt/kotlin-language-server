        ...
        init {
                environment =
                        KotlinCoreEnvironment.createForProduction(
                                ...
                                // Not to be confused with the CompilerConfiguration in the language
                                // server
                                // Configuration
                                configuration =
                                        KotlinCompilerConfiguration().apply {
                                                val langFeatures =
                                                        mutableMapOf<
                                                                LanguageFeature,
                                                                LanguageFeature.State>()
                                                for (langFeature in LanguageFeature.values()) {
                                                        langFeatures[langFeature] =
                                                ...
                                                // Fix: Enforce JVM 17 Target to properly resolve
                                                // modern
                                                // AndroidX interfaces with default methods (e.g.
                                                // LifecycleOwner, SavedStateRegistryOwner)
                                                put(
                                                        JVMConfigurationKeys.JVM_TARGET,
                                                        org.jetbrains.kotlin.config.JvmTarget.JVM_17
                                                )

                                                // configure jvm runtime classpaths
                                                configureJdkClasspathRoots()

                                                // Kotlin 1.8.20 requires us to specify the JDK
                                                // home, otherwise
                                                // java.* classes won't resolve
                                                // See
                                                // https://github.com/JetBrains/kotlin-compiler-server/pull/626
                                                val envJavaHome = System.getenv("JAVA_HOME")
                                                val jdkHome =
                                                        if (envJavaHome != null &&
                                                                        envJavaHome.isNotBlank()
                                                        )
                                                                File(envJavaHome)
                                                        else File(System.getProperty("java.home"))
                                                put(JVMConfigurationKeys.JDK_HOME, jdkHome)
                                                LOG.info("=== KLS Compiler Diagnostics ===")
                                                LOG.info("JDK_HOME: {}", jdkHome.absolutePath)
                                                LOG.info("JVM_TARGET: JVM_17")
                                                LOG.info(
                                                        "JAVA_HOME env: {}",
                                                        envJavaHome ?: "NOT SET"
                                                )
                                                ...
                                                LOG.info("Classpath entries (lifecycle-related):")
                                                classPath
                                                        .filter {
                                                                it.toString()
                                                                        .lowercase()
                                                        ...
                                                        .forEach { LOG.info("  LIFECYCLE: {}", it) }
                                                LOG.info("Classpath entries (savedstate-related):")
                                                classPath
                                                        .filter {
                                                                it.toString()
                                                                        .lowercase()
                                                        ...
                                                        .forEach {
                                                                LOG.info("  SAVEDSTATE: {}", it)
                                                        }
                                                LOG.info(
                                                        "Total classpath entries: {}",
                                                        classPath.size
                                                )
                                                LOG.info("=== End KLS Compiler Diagnostics ===")

                                                // Final-stage deduplication: extract artifact name
                                                // from filename
                                                // and keep only the highest version of each
                                                // artifact.
                                                // This catches duplicates that bypass upstream
                                                // deduplication.
                                                val versionedJarRegex =
                                                        Regex(
                                                                """(.+?)-(\d+\.\d+[\w.\-]*)(?:-api)?\.jar"""
                                                        )
                                                val dedupMap =
                                                ...
                                                // -> (path,
                                                // version)
                                                val nonJarPaths = mutableListOf<Path>()

                                                for (cp in classPath) {
                                                        val fileName =
                                                                cp.fileName?.toString()
                                                                        ?: cp.toString()
                                                        val match =
                                                                versionedJarRegex.matchEntire(
                                                                        fileName
                                                                )
                                                        if (match != null) {
                                                                val artifactName =
                                                                ...
                                                                // Non-versioned JARs, directories,
                                                                // etc. - keep as-is
                                                                nonJarPaths.add(cp)
                                                        }
                                                }

                                                val deduplicatedClassPath =
                                                        dedupMap.values.map { it.first } +
                                                                nonJarPaths
                                                LOG.info(
                                                        "Classpath deduplicated: {} -> {} entries",
                                                        classPath.size,
                                                        deduplicatedClassPath.size
                                                )

                                                addJvmClasspathRoots(
                                                        deduplicatedClassPath.map { it.toFile() }
                                                )
                                                addJavaSourceRoots(
                                                ...
                                                if (scriptsConfig.enabled) {
                                                        // Setup script templates (e.g. used by
                                                        // Gradle's Kotlin DSL)
                                                        val scriptDefinitions:
                                                                MutableList<ScriptDefinition> =
                                                                mutableListOf(
                                                        ...
                                                        if (scriptsConfig.buildScriptsEnabled &&
                                                                ...
                                                                LOG.info(
                                                                        "Configuring Kotlin DSL script templates..."
                                                                )
                                                                ...
                                                                try {
                                                                        ...
                                                                        // and its defaultImports
                                                                        // instead
                                                                        //       of
                                                                        // KotlinScriptDefinition.dependencyResolver
                                                                        // TODO: Use
                                                                        // ScriptDefinition.FromLegacyTemplate
                                                                        // directly if possible
                                                                        // scriptDefinitions =
                                                                        // scriptTemplates.map {
                                                                        // ScriptDefinition.FromLegacyTemplate(scriptHostConfig, scriptClassLoader.loadClass(it).kotlin) }
                                                                        ...
                                                                        LOG.error(
                                                                                "Error while loading script template classes"
                                                                        )
                ...
                val scriptDefinitions: List<ScriptDefinition> =
                        environment.configuration.getList(
                                ScriptingConfigurationKeys.SCRIPT_DEFINITIONS
                        )
                scriptDefinitions.takeIf { it.isNotEmpty() }?.let {
                        val annotations =
                                scriptDefinitions.flatMap {
                ...
                scripts =
                        ScriptDefinitionProvider.getInstance(project)!! as
                                CliScriptDefinitionProvider
        }

        fun updateConfiguration(config: CompilerConfiguration) {
                JvmTarget.fromString(config.jvm.target)?.let {
        ...
        fun createContainer(
                ...
                val container =
                        TopDownAnalyzerFacadeForJVM.createContainer(
                                ...
                                // TODO FileBasedDeclarationProviderFactory keeps indices, re-use it
                                // across
                                // calls
...
class Compiler(
        ...
        private val defaultCompileEnvironment =
                CompilationEnvironment(javaSourcePath, classPath, scriptsConfig)
        private val buildScriptCompileEnvironment =
                buildScriptClassPath
                        .takeIf {
                                it.isNotEmpty() &&
                                        scriptsConfig.enabled &&
                                        scriptsConfig.buildScriptsEnabled
                        }
        ...
        /**
         * Updates the compiler environment using the given configuration (which is a class from
         * this project).
         */
        fun updateConfiguration(config: CompilerConfiguration) {
        ...
        fun createKtFile(
                ...
                createPsiFile(content, file, language = KotlinLanguage.INSTANCE, kind = kind) as
                        KtFile
        ...
        fun compileKtFile(
                ...
                sourcePath: Collection<KtFile>,
                kind: CompilationKind = CompilationKind.DEFAULT
        ): Pair<BindingContext, ModuleDescriptor> = compileKtFiles(listOf(file), sourcePath, kind)
        ...
        fun compileKtFiles(
                ...
                if (kind == CompilationKind.BUILD_SCRIPT) {
                        // Print the (legacy) script template used by the compiled Kotlin DSL build
                        // file
                        files.forEach {
                                LOG.debug {
...
/** Simple semver comparison for classpath deduplication. Returns > 0 if v1 > v2. */
