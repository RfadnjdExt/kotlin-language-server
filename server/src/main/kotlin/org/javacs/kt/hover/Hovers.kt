...
fun hoverAt(file: CompiledFile, cursor: Int): Hover? {
        val refAndTarget = file.referenceAtPoint(cursor)
        if (refAndTarget != null) {
                val (ref, target) = refAndTarget
                val javaDoc = getDocString(file, cursor)
                val location = ref.textRange
                val hoverText = DECL_RENDERER.render(target)

                val markdownParts = mutableListOf("```kotlin\n$hoverText\n```")
                if (target is PackageViewDescriptor) {
                        markdownParts.add("*(package directive)*")
                }
                if (javaDoc.isNotEmpty()) {
                        markdownParts.add(javaDoc)
                }

                val hover = MarkupContent("markdown", markdownParts.joinToString("\n---\n"))
                val range =
                        Range(
        ...
        return typeHoverAt(file, cursor) ?: packageHoverAt(file, cursor)
}

private fun packageHoverAt(file: CompiledFile, cursor: Int): Hover? {
        val element = file.elementAtPoint(cursor) ?: return null
        val packageDirective =
                element as? KtPackageDirective
                        ?: element.parentsWithSelf
                                .filterIsInstance<KtPackageDirective>()
                                .firstOrNull()
                                ?: return null
        val packageName =
                packageDirective.fqName.asString().takeIf { it.isNotEmpty() } ?: return null
        val hoverText = "package $packageName"
        val hover =
                ...
                classifierNamePolicy =
                        object : ClassifierNamePolicy {
                                override fun renderClassifier(
                                        ...
                                        return ClassifierNamePolicy.SHORT.renderClassifier(
                                                classifier,
                                                renderer
                                        )
                                }
                ...
                .mapIndexed { i, it ->
                        ...
                        if (i == 0)
                                ret =
                                        it.substring(
                                                it.indexOf("/**") + 3
                                        ) // get rid of the start comment characters
                        else if (i == split.size - 1)
                                ret =
                                        it.substring(
                                                it.indexOf("*/") + 2
                                        ) // get rid of the end comment characters
...
private fun renderTypeOf(element: KtExpression, bindingContext: BindingContext): String? {
        ...
        if (smartCast != null && element is KtReferenceExpression) {
                val declaredType =
                        (bindingContext[BindingContext.REFERENCE_TARGET, element] as?
                                        CallableDescriptor)
                                ?.returnType
                if (declaredType != null) {
                        return result +
                                " (smart cast from " +
                                TYPE_RENDERER.renderType(declaredType) +
                                ")"
                }
        }
        return result
